...
| `src/WinMain.cpp` | Entry → `runApp` |
| `src/ui/MainWindow.cpp` | The whole app (window, toolbar, tree, editor, dock, build/run, project) |
| `src/ui/Theme.h` | Palette — **code embodiment of DESIGN.md** (dark/coral, OS follow, diag + trust + dialog helpers) |
| `src/ui/SettingsDialog.{h,cpp}` | Themed modal Settings dialog |
| `src/ui/ProjectSettingsDialog.{h,cpp}` | Themed modal **Project Settings** form over `sentinel.toml` |
| `src/ui/SigningDialog.{h,cpp}` | Themed modal **Signing & Trust** panel (ADR-0061 keygen/sign/verify + trust viewer) |
| `src/ui/AboutDialog.{h,cpp}` | Themed modal **About** box (S2 shield + name/version/tagline + lines-of-code shields.io badges) |
| `src/ui/PasswordDialog.{h,cpp}` | Themed modal password prompt (one field, or two with a match check for sealing) |
...
27. **Rename to "Sentinel-IDE" + git/repo prep.** Display name → **Sentinel-IDE** (titlebar/`kAppName`, About box + caption, `.rc` ProductName/FileDescription/InternalName/OriginalFilename, `app.manifest` identity) and the **exe → `Sentinel-IDE.exe`** via CMake `OUTPUT_NAME` (the CMake target id, window class `SentinelIDEMainWindow`, `sentinelide` namespace, `%LOCALAPPDATA%\SentinelIDE` settings dir, and the `G:\SentinelIDE` folder stay "SentinelIDE" — so saved settings + the `-Class` capture path keep working). `launch.ps1`/`capture.ps1` now match the **process name** `Sentinel-IDE`; the file associations were re-registered to the new exe. **Repo prepped (kept private):** added `LICENSE` (verbatim GPL-3.0, from SQLTerminal-Win32), a public `README.md`, and `.gitignore` (excludes `build/`, `*.sealed`/`*.key`, `.claude/`, `_bmad/`); `git init -b main` + initial commit (`c812621`, 92 files — source/docs/examples/packaging/scripts/art + the `_bmad-output` design docs; no secrets/build/tooling). Gotcha: the repo is on a network share, so git needed `safe.directory` added. **Not yet pushed** — `gh` isn't installed; create a private `Sentinel-IDE` repo on GitHub and `git remote add origin … && git push -u origin main` (or `gh repo create Sentinel-IDE --private --source=. --remote=origin --push`).
