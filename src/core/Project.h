...
// The manifest reader is Sentinel now — parse_manifest in parsers.sentinel does both
// the flat `[section] key=value` fields AND the `[[target]]` blocks in one pass,
// replacing the Win32 profile API (projIni/projArr) and the hand-written parseTargets.
// It faithfully reproduces both behaviours, including the case-insensitivity asymmetry
// (flat fields case-insensitive like GetPrivateProfileString; target field keys
// case-sensitive like parseTargets). Byte-identical to the C++ #else fallback below —
// held in lockstep by tests/manifest_xcheck.cpp (14 cases). The defaults and the
// single-target fallback stay host-side (they are trivial and enum-typed); Sentinel
// returns raw found-flag + string values, we apply typeFromName/tierFromName/defaults.
// File I/O stays native (readUtf8), as with the other parsers.
#include "sentinel_parsers.h"   // generated by snc: parse_manifest() + sentinel_free_bytes()
inline SentinelProject loadProject(const std::wstring& folder) {
    ...
    size_t pos = 0;
    auto rd8 = [](const uint8_t* q) { uint64_t v = 0; for (int i = 0; i < 8; i++) v |= (uint64_t)q[i] << (8 * i); return v; };
    auto lp = [&]() -> std::wstring {
        uint64_t nlen = rd8(out + pos); pos += 8;
        std::wstring w;
        if (nlen > 0) { int fw = MultiByteToWideChar(CP_UTF8, 0, (const char*)out + pos, (int)nlen, nullptr, 0); w.resize((size_t)fw); MultiByteToWideChar(CP_UTF8, 0, (const char*)out + pos, (int)nlen, w.data(), fw); }
        pos += (size_t)nlen; return w;
    };
    auto scalar = [&](std::wstring& dst) { bool found = (pos < (size_t)olen && out[pos] == 1); pos++; std::wstring v = lp(); if (found) dst = v; };
    auto arr = [&]() { std::vector<std::wstring> v; while (pos < (size_t)olen && out[pos] == 1) { pos++; v.push_back(lp()); } if (pos < (size_t)olen) pos++; return v; };

    p.name = projBase(folder); scalar(p.name);
    p.version = L"0.1.0"; scalar(p.version);
    std::wstring typ = L"executable"; scalar(typ); p.type = typeFromName(typ);
    p.entry = L""; scalar(p.entry);
    p.icon = L""; scalar(p.icon);
    p.srcDir = L"src"; scalar(p.srcDir);
    std::wstring tier = L"experimental"; scalar(tier); p.defaultTier = tierFromName(tier);
    p.signRequire = L"off"; scalar(p.signRequire);
    p.trust = L"sentinel-trust.toml"; scalar(p.trust);
    std::wstring so = L"false"; scalar(so); p.signOutput = (so == L"true" || so == L"1");
    p.libPaths = arr();
    p.links = arr();
    while (pos < (size_t)olen && out[pos] == 1) {
        pos++;
        Target t;
        t.name = lp(); t.entry = lp();
        std::wstring tt = lp(); t.type = typeFromName(tt);
        while (pos < (size_t)olen && out[pos] == 1) { pos++; t.links.push_back(lp()); }
        if (pos < (size_t)olen) pos++;   // links terminator
