...
inline void saveSettings(const Settings& s) {
    ...
    // [editor] d2d chose between the Direct2D editor and RichEdit; phase 46 slice 8 deleted
    // the RichEdit editor, so the key means nothing. Nothing READS it any more — that alone
    // is what makes a stale `d2d=0` from 0.1.8/0.1.9 harmless rather than an editor-less
    // window — and passing nullptr as the value DELETES it, so the file stops carrying a
    // setting the program no longer has. Safe to drop this line once 0.1.10 is old news.
    WritePrivateProfileStringW(L"editor", L"d2d", nullptr, path.c_str());
